...
; Single source of truth: version.txt. build-setup.ps1 passes /DMyAppVersion and
; /DMyAppVersionInfo; the values below are only in-sync fallbacks for manual ISCC runs.
#ifndef MyAppVersion
  #define MyAppVersion "2026.9.9"
#endif
#ifndef MyAppVersionInfo
  #define MyAppVersionInfo "2026.9.9.0"
#endif
...
[Icons]
; Connect QR shortcuts are created by install-hermes.ps1 (-CreateShortcuts) AFTER
; connect.html exists; [Icons] entries here would be skipped (createonlyiffileexists
; at ssInstall time) or duplicate them in a different Start Menu folder.
...
var
  Lines, Secrets: TStringList;
  InstallPs1, Ws, AppDir, MemMode, ProviderUrl, ProviderKey, ProviderModel, SecretsPath: string;
begin
  InstallPs1 := ExpandConstant('{app}\installer\install-hermes.ps1');
  AppDir := ExpandConstant('{app}');
  Ws := WorkspacePage.Values[0];
  SecretsPath := ExpandConstant('{tmp}\hermes-install-secrets.ps1');
  ...
  // The MemOS API key must NOT be inlined into the launch script (it would persist
  // in {tmp} as plaintext for the whole install). It goes to a separate secrets file
  // that the launch script sources and deletes immediately on start.
  if Trim(ProviderKey) <> '' then
  begin
    Secrets := TStringList.Create;
    try
      Secrets.Add('$env:HERMES_SETUP_MEMOS_PROVIDER_KEY = ''' + EscapePsSingleQuoted(ProviderKey) + '''');
      SaveStringListUtf8(SecretsPath, Secrets);
    finally
      Secrets.Free;
    end;
  end;
  ...
  try
    Lines.Add('# Auto-generated by HermesWorkspaceSetup - do not edit (contains no secrets)');
    Lines.Add('$ErrorActionPreference = ''Stop''');
    Lines.Add('$secretsFile = ''' + EscapePsSingleQuoted(SecretsPath) + '''');
    Lines.Add('if (Test-Path -LiteralPath $secretsFile) {');
    Lines.Add('  . $secretsFile');
    Lines.Add('  Remove-Item -LiteralPath $secretsFile -Force -ErrorAction SilentlyContinue');
    Lines.Add('}');
    // Capture into a plain variable and clear the env var BEFORE spawning the
    // installer, so long-running children (gateway, pnpm) never inherit the key.
    Lines.Add('$memosProviderKey = $env:HERMES_SETUP_MEMOS_PROVIDER_KEY');
    Lines.Add('Remove-Item Env:HERMES_SETUP_MEMOS_PROVIDER_KEY -ErrorAction SilentlyContinue');
    Lines.Add('try {');
    ...
    if Trim(ProviderKey) <> '' then
      Lines.Add('    -MemOSProviderKey $memosProviderKey `');
